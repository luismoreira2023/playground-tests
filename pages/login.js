...
class Login {
  constructor(page) {
    ...
    this.regularLogin = page
      .locator('p')
      .filter({ hasText: /^Login: teste$/ })
      .getByRole('strong');
    ...
    this.blockedLogin = page
      .locator('p')
      .filter({ hasText: 'Login: testeblock' })
      .getByRole('strong');
    ...
    this.logoutMessage = page.getByText(
  ...
  async validateInstructions() {
    await expect(this.instructionsTitle).toHaveText(
      PAGE_TEXT.login.instructions.title
    );
    await expect(this.rule1).toHaveText(PAGE_TEXT.login.instructions.firstRule);
    await expect(this.rule2).toHaveText(
      PAGE_TEXT.login.instructions.secondRule
    );
