...
class Table {
  ...
  getCharNameLocator(character) {
    return this.page.locator(
      '#tableCharacterName' + character.replace(' ', '')
    );
  }

  getHouseNameLocator(character) {
    return this.page.locator(
      '#tableCharacterHouse' + character.replace(' ', '')
    );
  }

  getBirthDateLocator(hpCharacter) {
    return this.page.locator(
      '#characterDateOfBirth-' + hpCharacter.replace(' ', '')
    );
  }
  ...
  async validateInstructions() {
    ...
    await expect(this.rule2).toHaveText(
      PAGE_TEXT.table.instructions.secondRule
    );
  ...
  async validateBirthDate(hpCharacter, dateOfBirth) {
    await expect(this.getBirthDateLocator(hpCharacter)).toHaveText(
